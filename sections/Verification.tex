...
Later in the section I attempt to verify these properties on the Hardware and
Software system that I outlined in Section 2 and Section 3.
These properties are analyzed using CBMC or a new framework known as ILA, both
of which I will be explaining.

All properties are being run on the Read-Only Firmware of Verified boot, in the
unmodified vboot\_reference repository provided by Google. 
When secure promises cross the Hardware-Firmware boundary, they are evaluated
using the Hardware I selected in Section 2, using Firmware that I created which
modelled the Firmware originally found in Google's Coreboot repository. 

...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.9\linewidth]{full_flow.png}
  \caption[Vboot Full Call Graph]{The call graph of functions for all of Verified Boot}\label{fig:loadfw}
\end{figure}
...
\subsection{TPM Send and Receive}
...
\begin{table}[!htbp]
    \centering
    \caption{CBMC tests on function VbExTpmSendReceive}\label{sfw_results}
    \begin{tabular}{llllll}
        \toprule
        test name & steps & VCCs & time (s) & sat/unsat  \\ \bottomrule
        TPM Liveness & & & & \\
...
\begin{table}[!htbp]
    \centering
    \caption{CBMC tests on TLCL Library}\label{sfw_results}
    \begin{tabular}{llllll}
        \toprule
        test name & steps & VCCs & time (s) & sat/unsat  \\ \bottomrule
        TLCL Error Handling& & & & \\
